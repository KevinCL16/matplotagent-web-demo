...
for i, year in enumerate(years):
    ...
    wedges, texts = ax.pie(values, radius=radius, colors=color_list, startangle=90, counterclock=False,
                           wedgeprops=dict(width=size, edgecolor='w'))
    
    # Annotate each segment with its market share percentage
    for j, p in enumerate(wedges):
        ang = (p.theta2 - p.theta1)/2. + p.theta1
        y = radius * 0.5 * (1 + 0.5 * size) * np.sin(np.deg2rad(ang))
        x = radius * 0.5 * (1 + 0.5 * size) * np.cos(np.deg2rad(ang))
    ...
    white_patch = patches.Wedge((0, 0), radius, 90, 90, width=size, facecolor='white', edgecolor='w')
    ax.add_patch(white_patch)
    ax.text(0, radius - size/2, year, ha='center', va='center', fontsize=10, color='black', weight='bold')
