...
def dendrogram_coord_convert(Z):
    icoord, dcoord = hierarchy.dendrogram(Z, no_plot=True)
    theta = np.deg2rad((icoord[:, 0] + icoord[:, 1]) / 2)
    radius = dcoord[:, 2]
    return theta, radius
...
cutree = hierarchy.cut_tree(Z, n_clusters=4)  # Example clustering result

# Plot with color-coded clusters
fig, ax = plt.subplots(subplot_kw={'projection': 'polar'})
for cluster in np.unique(cutree):
    mask = (cutree == cluster).flatten()
    ax.scatter(theta[mask], radius[mask], label=f'Cluster {cluster + 1}')
...
for angle, label in zip(perimeter_angles, perimeter_labels):
    ax.text(angle, ax.get_ylim()[1] + 0.1, label, ha='center', va='center')

# Design a correlation axis (not implemented in this example)

ax.set_title('Circular Dendrogram with Clusters')
ax.legend()
